...
def find_files(directory: str) -> list[str]:
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            ...
            if file.endswith(".py") or file in {"Makefile", ".python-version"}:
...
def handle_file_edited(context: Context, file: File) -> Context:
    ...
    if not file.path.startswith(context.ticket.repository):
        context.errors.append(f"Attempted to edit file outside of repository.{{'file_path': '{file.path}', 'repository': '{context.ticket.repository}'}}")
        return context
    ...
    if path.name in disallowed_files:
        context.errors.append(f"Attempted to edit disallowed file.{{'file_path': '{file.path}'}}")
        return context
...
def run_make_target(target, makefile_dir=None, makefile_name="Makefile") -> SubprocessResult:
    """
...
def parse_llm_response(response, context: Context) -> tuple[tuple[Context, LLMResponse], ValidationError | None]:
    """
    ...
    validation_error = None
    tool_calls: list[ToolCall] = [ToolCall(name=tc.function.name, arguments=tc.function.arguments) for tc in response.message.tool_calls or []]
...
def run_bug_fix_agent(max_iterations: int = 5):
    ...
    while True:
        ...
        prompt = Prompt(
            ...
            available_tools=[
                'get_ticket', 'find_files', 'read_file', 'search_web',
                'edit_file', 'validate_fix', 'format_code',
            ],
        )
        ...
        (context, llm_response), validation_error = parse_llm_response(response, context)
        if validation_error is not None:
            # try again
            iteration += 1
            continue
        ...
        if not llm_response.tool_calls:
            context.errors += ["You decided to not use any tools, but the bug fix has not been validated. Use the available tools to fix the bug."]
            iteration += 1
            continue
            ...
            match tool.function.name:
                ...
                case 'find_files':
                    if context.ticket.repository == "":
                        context.errors.append("You must retrieve the ticket before finding files.")
                        break
